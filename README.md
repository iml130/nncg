...
called. This creates a new subgraph for every node that is added as content (an edge with name "content")
...
Some nodes replace themselves with the new subgraph. However, this may change in future as the
principle of lowering is to never lose meta information and, if possible, add meta information when
performing operations that could add complexity to the graph.
An example is the loop unrolling optimization (function `unroll()` in `LoopNode`).
...
and tested. Tests are performed by using the provided images.
